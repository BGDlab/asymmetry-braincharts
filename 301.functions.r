...
gamlssWrapper <- function( Model, Data, Ctrl, Method='RS',... ) { ##
  ## The problem:
...
Refit.Function <- function(idx=1, List) {
  ...
  #HOLDER$SUBSET = HOLDER$SUBSET %>% filter(!study_site %in% TMP1[TMP1$N<10,]$study_site)
  #HOLDER$SUBSET = subset(HOLDER$SUBSET, !(rh.GM.lingualTransformed == 0))
  
  EXTRACT = readRDS(paste0(PATHS.LIST$FIT.EXTRACT,'/',List[[idx]]$model))
  
  if( is.null(EXTRACT$diagnostics) ) {
    if( (HOLDER$MODEL$inc.fp ) || (grepl(".fp.",List[[idx]]$model,fixed=TRUE)) ) {
      GAMLSS.CTRL <- gamlss.control(n.cyc=500) # Setting this high - if fp has fit, we really want bfp to have the best change
      ...
      if(!(is.null(EXTRACT.bfp$diagnostics))) {
        EXTRACT.bfp <- Extract.Wrapper( HOLDER, Fit.Full=STORE.OPT$Fit.Full, start.from=EXTRACT$FIT, GAMLSS.METHOD='CG') ## helpful to start.from, improves convergence speed
        print('Using CG for bfp')
      }
...
Extract.Wrapper <- function( Holder, Store.Full=FALSE,GAMLSS.METHOD='RS',... ) {
  ...
  FIT.FULL <- try(gamlssWrapper(Model=Holder$MODEL,
                                ...
                                Ctrl=GAMLSS.CTRL, Method=GAMLSS.METHOD,
                                ... ),
                  silent=TRUE )
...
Boot.Function <- function( n, Base.Seed, Holder, Create.INIT=TRUE ) {
  ...
  if(is.null(Holder$MODEL$stratify)) {
    ...
    LAPPLY <- lapply(SPLIT,function(X){sample(x=X,size=length(X),replace=TRUE)}) #tells us which subjects we want
  ...
  if(!(is.null(EXTRACT$diagnostics))) {
    print('Using CG for fp')
    EXTRACT <- Extract.Wrapper(Holder, Store.Full=FALSE, start.from=INIT , GAMLSS.METHOD='CG') ## helpful to start.from, improves convergence speed
  }
...
LOO.Function <- function( n, Base.Seed, Holder, Create.INIT=TRUE ) {
  source("100.common-variables.r")
  source("101.common-functions.r")
  
  source("300.variables.r")
  source("301.functions.r")
  ## must source file within LOO.FUNCTION, otherwise contents not available witihn %dopar%
  
  set.seed( seed=Base.Seed )
  
  LOO.SETS <- do.call( what=expand.grid, args=lapply(Holder$SUBSET[ Holder$MODEL$covariates$RANEF ], levels) )
  
  LIST <- list()
  for( LAB in Holder$MODEL$covariates$RANEF ) {
    LIST[[LAB]] <- Holder$SUBSET[, LAB ] != LOO.SETS[ n, LAB ] ## here the "n" argument is used to select the RANEF
    ## we're doing leave-one-out, so we use not equal (!=)
  }
  INDEX <- which(Reduce(f=`&`,x=LIST))
  
  if( length(INDEX)==0 ) { return(NA) }
  
  Holder$SUBSET <- Holder$SUBSET[INDEX,] ## replace SUBSET with LOO-SUBSET
  
  ##
  ## In the following code we are creating an initialisation block to use within the LOO-refit
  ## NOTE: this must be done within this function, since it depends on the LOO sample drawn
  if( Create.INIT ) {
    tPRED <- Apply.Param(NEWData=Holder$SUBSET, FITParam=Holder$FIT.EXTRACT$param,
                         Pred.Set=NULL, Prefix="", Add.Moments=FALSE, Add.Normalise=FALSE, Add.Derivative=FALSE, MissingToZero=TRUE,
                         verbose=FALSE )
...
Apply.Param <- function(NEWData, FITParam,
  ...
  for( wPARAM in Model.Parameters ) {
    ...
    if(any(is.na(Fit.Fixef))){
      print('This subset of data has missing levels. Have set missing levels to zero!')
      Fit.Fixef[is.na(Fit.Fixef)] = 0 # subsets of data my drop levels, make sure to 
    }
...
Find.Models.To.Fit <- function( Paths.List ) {
  ...
  for( IDX in 1:length(Model.Set.notbfp) ) {
    if( file.exists( file.path(Paths.List$FIT.EXTRACT,Model.Set.notbfp[IDX]) ) ) {
      if( STORE.OPT$Fit.Overwrite=="refit.failed" ) {
        EXTRACT = readRDS(file.path(Paths.List$FIT.EXTRACT,Model.Set.notbfp[IDX]))
        if (!(is.null(EXTRACT$diagnostics))) {
          STR <- sprintf("%s %s Existing model output failed, try re-running [OVERWRITE].",Paths.List$Tag, Model.Set.notbfp[IDX])
          cat(STR,"\n")
          warning(STR)
        }else{
          STR <- sprintf("%s %s Model already has an existing fit-extract output that successfully ran [SKIP].",Paths.List$Tag, Model.Set.notbfp[IDX])
          warning(STR)
          cat(STR,"\n")
...
ValidateCleanInput <- function( IN=NULL, Reference.Subset=NULL, Reference.Model=NULL, Reference.Param=NULL ) {
  ...
  #LOGICAL[["KNOWN.RANEF"]] <- ( ! FIND.MISSING$Obs ) ## Change back to this
  LOGICAL[["KNOWN.RANEF"]] <- ifelse(is.null( FIND.MISSING$Obs ),NA,( ! FIND.MISSING$Obs ) )
...
Fit.Function <- function(idx=1, List) {
  ...
  if(!(is.null(EXTRACT$diagnostics))) {
    print('Using CG for fp')
    EXTRACT <- Extract.Wrapper( HOLDER, Store.Full=TRUE, GAMLSS.METHOD='CG') ## helpful to start.from, improves convergence speed
  }
